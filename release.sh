...
GOOS=darwin GOARCH=amd64 go build -o release/dscda-darwin

# Fetch deps and build Linux release
echo Building Linux binary...
GOOS=linux GOARCH=amd64 go get ./...
GOOS=linux GOARCH=amd64 go build -o release/dscda-linux 

# Fetch deps and build Windows release
echo Building Windows binary...
GOOS=windows GOARCH=amd64 go get ./...
GOOS=windows GOARCH=amd64 go build -o release/dscda-windows.exe 
...
echo -e "\nArtifacts in release directory can now be uploaded to https://github.com/datastax-cda/workshop-builder/releases"
...
echo "  File: dscda-cli.rb"
echo "  Version: ${RELVER}"
echo "  SHA256: `openssl dgst -sha256 release/dscda-darwin | cut -d\  -f2`"
